...
func main() {
	rr := newPathResolver2()
	rr.Add("GET /hello", hello3)
	rr.Add("(GET|HEAD) /goodbye(/?[A-Za-z0-9]*)?", goodbye3)
	http.ListenAndServe(":8080", rr)
}

func newPathResolver2() *regexResolver {
...
func hello3(res http.ResponseWriter, req *http.Request) {
	query := req.URL.Query()
	name := query.Get("name")
	if name == "" {
		name = "Inigo Montoya"
...
func goodbye3(res http.ResponseWriter, req *http.Request) {
	path := req.URL.Path
